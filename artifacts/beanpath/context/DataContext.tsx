...
export type CherryDelivery = {
  ...
  receiptNo: string;
  cherryRegisterNo: string;
  deliveryReportNo: string;
  quantityBidons: number;
  pricePerBidonFC: number;
  totalFC: number;
...
function rowToStation(r: any): Station {
  return { id: r.id, name: r.name, territoire: r.territoire, groupement: r.groupement, coopId: r.coop_id };
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToFarmer(r: any): Farmer {
  return {
    id: r.id, bioId: r.bio_id, firstName: r.first_name, lastName: r.last_name,
    phone: r.phone, age: r.age, gender: r.gender,
    territoire: r.territoire, groupement: r.groupement, village: r.village,
    stationId: r.station_id, coopId: r.coop_id, nbPieds: r.nb_pieds,
    groupRole: r.group_role, crop: r.crop, registeredAt: r.registered_at,
  };
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToDelivery(r: any): CherryDelivery {
  return {
    id: r.id, farmerId: r.farmer_id, farmerName: r.farmer_name, farmerBioId: r.farmer_bio_id,
    stationId: r.station_id, stationName: r.station_name, groupement: r.groupement, village: r.village,
    purchaseDate: r.purchase_date, receptionDate: r.reception_date, receiptNo: r.receipt_no,
    cherryRegisterNo: r.cherry_register_no, deliveryReportNo: r.delivery_report_no,
    quantityBidons: r.quantity_bidons, pricePerBidonFC: r.price_per_bidon_fc,
    totalFC: r.total_fc, exchangeRateFC_USD: r.exchange_rate_fc_usd,
    paymentMethod: r.payment_method, notes: r.notes, synced: r.synced, recordedAt: r.recorded_at,
  };
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToRegister(r: any): CherryRegister {
  return {
    id: r.id, registerNo: r.register_no, stationId: r.station_id,
    deliveryReportNo: r.delivery_report_no, date: r.date,
    groupement: r.groupement, village: r.village,
    totalBidons: r.total_bidons, totalFC: r.total_fc,
    deliveryIds: r.delivery_ids ?? [],
  };
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToReport(r: any): DeliveryReport {
  return {
    id: r.id, reportNo: r.report_no, stationId: r.station_id,
    dateFrom: r.date_from, dateTo: r.date_to,
    totalBidons: r.total_bidons, totalFC: r.total_fc,
    registerNos: r.register_nos ?? [],
  };
}
...
function rowToLot(r: any): Lot {
  return {
    id: r.id, ref: r.ref, crop: r.crop, stage: r.stage,
    weightKg: r.weight_kg, bidonCount: r.bidon_count, farmerCount: r.farmer_count,
    harvestSeason: r.harvest_season, certifications: r.certifications ?? [],
    openedAt: r.opened_at, currentOrgId: r.current_org_id, stationId: r.station_id,
    sourceRegisterNos: r.source_register_nos ?? [],
    cupScore: r.cup_score, processingMethod: r.processing_method,
  };
}
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const load = useCallback(async () => {
    // Try Supabase first; fall back to AsyncStorage / seed data
    try {
      const [stRes, fRes, dRes, rRes, rpRes, lRes] = await Promise.all([
        supabase.from("stations").select("*"),
        supabase.from("farmers").select("*"),
        supabase.from("cherry_deliveries").select("*"),
        supabase.from("cherry_registers").select("*"),
        supabase.from("delivery_reports").select("*"),
        supabase.from("lots").select("*"),
      ]);

      const hasData =
        stRes.data && stRes.data.length > 0 &&
        fRes.data  && fRes.data.length  > 0;

      if (hasData && !stRes.error && !fRes.error) {
        const sts  = stRes.data!.map(rowToStation);
        const fs   = fRes.data!.map(rowToFarmer);
        const ds   = dRes.data  && !dRes.error  ? dRes.data.map(rowToDelivery)  : [];
        const rs   = rRes.data  && !rRes.error  ? rRes.data.map(rowToRegister)  : [];
        const rps  = rpRes.data && !rpRes.error ? rpRes.data.map(rowToReport)   : [];
        const ls   = lRes.data  && !lRes.error  ? lRes.data.map(rowToLot)       : [];

        setStations(sts);  setFarmers(fs);   setDeliveries(ds);
        setRegisters(rs);  setReports(rps);  setLots(ls);

        // Update local cache
        AsyncStorage.setItem(STATIONS_KEY,   JSON.stringify(sts));
        AsyncStorage.setItem(FARMERS_KEY,    JSON.stringify(fs));
        AsyncStorage.setItem(DELIVERIES_KEY, JSON.stringify(ds));
        AsyncStorage.setItem(REGISTERS_KEY,  JSON.stringify(rs));
        AsyncStorage.setItem(REPORTS_KEY,    JSON.stringify(rps));
        AsyncStorage.setItem(LOTS_KEY,       JSON.stringify(ls));
        return;
      }
    } catch {
    ...
    // Fall back to local cache or seed data
  ...
  const addFarmer = useCallback(async (f: Omit<Farmer, "id" | "registeredAt">) => {
    ...
    const next: Farmer = { ...f, id: "f" + Date.now(), registeredAt };

    // Insert to Supabase (best-effort — don't block local update on failure)
    try {
      await supabase.from("farmers").insert({
        id: next.id, bio_id: f.bioId, first_name: f.firstName, last_name: f.lastName,
        phone: f.phone, age: f.age, gender: f.gender,
        territoire: f.territoire, groupement: f.groupement, village: f.village,
        station_id: f.stationId, coop_id: f.coopId, nb_pieds: f.nbPieds,
        group_role: f.groupRole, crop: f.crop, registered_at: registeredAt,
      });
    } catch { /* offline — will sync later */ }

    setFarmers(prev => {
      const u = [next, ...prev];
      AsyncStorage.setItem(FARMERS_KEY, JSON.stringify(u));
      return u;
    });
    return next;
  }, []);
  ...
  const addDelivery = useCallback(async (d: Omit<CherryDelivery, "id">) => {
    ...
    // Insert to Supabase (best-effort — offline writes get synced by SyncContext)
    try {
      await supabase.from("cherry_deliveries").insert({
        id: next.id, farmer_id: d.farmerId, farmer_name: d.farmerName, farmer_bio_id: d.farmerBioId,
        station_id: d.stationId, station_name: d.stationName, groupement: d.groupement, village: d.village,
        purchase_date: d.purchaseDate, reception_date: d.receptionDate, receipt_no: d.receiptNo,
        cherry_register_no: d.cherryRegisterNo, delivery_report_no: d.deliveryReportNo,
        quantity_bidons: d.quantityBidons, price_per_bidon_fc: d.pricePerBidonFC,
        total_fc: d.totalFC, exchange_rate_fc_usd: d.exchangeRateFC_USD,
        payment_method: d.paymentMethod, notes: d.notes, synced: true, recorded_at: d.recordedAt,
      });
      // Mark as synced locally after successful DB write
      next.synced = true;
    } catch { /* offline — SyncContext.triggerSync will push it later */ }

    setDeliveries(prev => {
      const u = [next, ...prev];
      AsyncStorage.setItem(DELIVERIES_KEY, JSON.stringify(u));
      return u;
    });
    return next;
  }, []);
